...
/**
* This file contains the default settings for learns
* Do not edit this file as it may change in future revisions of the software.
* Correct procedure to setup Learns is the following:
...
$config['rootdir'] = getcwd(); //dirname(__FILE__); // This is the physical disk location for your Learns installation. Normally you don't have to touch this setting.
// If you use IIS then you MUST enter the complete rootdir e.g. : $rootDir="C:\Inetpub\wwwroot\Learns"!
// Some IIS installations also require to use forward slashes instead of backslashes, e.g.  $rootDir="C:/Inetpub/wwwroot/Learns"!
// If you use OS/2 this must be the complete rootdir with FORWARD slashes e.g.: $rootDir="c:/Learns";!


// Site Info
$config['sitename']           = 'Learns'; // The official name of the site (appears in the Window title)
$config['scriptname']         = 'admin.php'; // The name of the admin script

$config['defaultuser']        = 'admin'; // This is the default username when Learns is installed
$config['defaultpass']        = 'password'; // This is the default password for the default user when Learns is installed
...
$config['deletenonvalues']           = 1; // By default, Learns does not save responses to conditional questions that haven't been answered/shown. To have Learns save these responses change this value to 0.
$config['stringcomparizonoperators'] = 0; // By default, Learns assumes the numrical order for comparizon operators in conditions. If you need string comparizon operators, set this parameter to 1
...
$config['memory_limit'] = '256'; // This sets how much memory Learns can access in megabytes. 256 MB is the minimum recommended - if you are using PDF functions up to 512 MB may be needed
...
// The user can call the Learns login at /Learns/admin and pass username and
...
// the webserver will be trusted by Learns and used for authentication
// unless a username mapping is used see auth_webserver_user_map below
//
// The user still needs to be defined in the Learns database in order to
...
// you created for the administration of  Learns and if you want to
...
// If this option is set to true, then Learns operators will only 'see'
// users that belong to at least one of their groups
// Otherwise they can see all operators defines in Learns
...
// demoMode
// If this option is set to true, then Learns will go into demo mode.
...
* Learns installation package are read-only. If you want to modify a theme just copy it first.
...
/**
*  $chartfontfile - set the font file name used to created the charts in statistics - this font must reside in <Learns root folder>/fonts
*  Set this to specific font-file (for example 'DejaVuSans.ttf') or set it to 'auto' and Learns tried to pick the best font depending on your survey base language
*/
$config['chartfontfile'] = 'auto';
/**
*  $alternatechartfontfile - array of the font file name used to created the charts in statistics in specific language - this font must reside in <Learns root folder>/fonts
*  Only used if $chartfontfile is set to auto. If font file doesn't exist in <Learns root folder>/fonts, an alert is sent to admin
*/
...
/**
* $updatecheckperiod - sets how often Learns checks for updates - the number sets the number of days between updates.
...
/**
* @var $force_ssl string - forces Learns to run through HTTPS or to block HTTPS
*   'on' =  force SSL/HTTPS to be on (This will cause Learns
...
* to the http part of your normal Learns URL.
*   e.g. https://your.domain.org/Learns/admin/admin.php
* If Learns comes up as normal, then everything is fine. If you
...
/**
* Sets if any part of Learns may be embedded in an iframe
...
* default Learns username is limited to 2000 credits/hour and 30 000 crdits/day see : http://www.geonames.org/export/ Terms and conditions
*/
$config['GeoNamesUsername'] = 'learns';
 ...
 * Example : https://github.com/Learns/Learns/blob/master/framework/utils/fileExtensions.php
 */
...
/**
* This variable defines the languages available in Learns (front- and backend)
...
//Learns installation. Do not change unless you know what you are doing.
...
$config['tempdir']                  = $config['rootdir'] . DIRECTORY_SEPARATOR . "tmp"; // The directory path where Learns can store temporary files
